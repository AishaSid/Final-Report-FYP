...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/text\_extraction\_service.py}} :: \texttt{\seqsplit{TextExtractionService.chunk\_text}} / \texttt{\seqsplit{\_create\_text\_chunk}}
    \end{itemize}
    \item Embedding generation and vector storage
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/embedding\_service.py}} :: \texttt{\seqsplit{EmbeddingService.generate\_single\_embedding}}, \texttt{\seqsplit{generate\_embeddings}}, \texttt{\seqsplit{store\_text\_chunks\_with\_embeddings}}
        \item \texttt{\seqsplit{backend/app/services/chroma\_service.py}} :: \texttt{\seqsplit{ChromaService.add\_embeddings}}, \texttt{\seqsplit{search\_similar}}
        \item \texttt{\seqsplit{backend/app/repositories/text\_chunk\_repository.py}} :: \texttt{\seqsplit{update\_embedding}}, \texttt{\seqsplit{get\_chunks\_without\_embeddings}}, \texttt{\seqsplit{vector\_search}} / \texttt{\seqsplit{vector\_search\_chroma}}
    \end{itemize}
    \item Context retrieval (semantic similarity)
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/rag\_context\_service.py}} :: \texttt{\seqsplit{RAGContextService.assemble\_context}}
        \item \texttt{\seqsplit{backend/app/services/search\_service.py}} :: \texttt{\seqsplit{SearchService.semantic\_search}}, \texttt{\seqsplit{find\_similar\_content}}
    \end{itemize}
    \item AI response generation (RAG orchestration and assistant)
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/ai\_chat\_service.py}} :: \texttt{\seqsplit{AIChatService.chat}}
        \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{CopilotService.generate\_suggestion}}
    \end{itemize}
\end{itemize}
}
...
{\small
\begin{enumerate}
    \item User uploads file via endpoints in \texttt{\seqsplit{backend/app/api/v1/endpoints/files.py}}.
    \item File processing triggers text extraction and chunking:
    \begin{itemize}
        \item \texttt{\seqsplit{TextExtractionService.extract\_text\_from\_file(...)}} and \texttt{\seqsplit{TextExtractionService.chunk\_text(...)}}
    ...
    \begin{itemize}
        \item \texttt{\seqsplit{EmbeddingService.generate\_embeddings(...)}} / \texttt{\seqsplit{generate\_single\_embedding(...)}}
    ...
    \begin{itemize}
        \item \texttt{\seqsplit{RAGContextService.assemble\_context(...)}} calls embedding/search services to retrieve top-$k$ chunks.
        \item \texttt{SearchService} handles hybrid/fallback logic (ChromaDB preferred, fallback to MongoDB vector search).
    \end{itemize}
    \item LLM generation:
    \begin{itemize}
        \item \texttt{\seqsplit{AIChatService.chat(...)}} or \texttt{\seqsplit{CopilotService.generate\_suggestion(...)}} build a prompt from the retrieved context and call the configured LLM (\texttt{GroqService} or other).
    \end{itemize}
...
{\small
\begin{enumerate}
    \item User uploads file $\rightarrow$ API endpoint in \texttt{\seqsplit{backend/app/api/v1/endpoints/files.py}} triggers \texttt{\seqsplit{process\_uploaded\_file}}.
    \item \texttt{\seqsplit{process\_uploaded\_file}} calls \texttt{\seqsplit{TextExtractionService.chunk\_text}} to create chunks.
    \item Indexing background task (or immediate synchronous call) calls \texttt{\seqsplit{EmbeddingService.generate\_embeddings}} for chunks.
    \item \texttt{EmbeddingService} stores embeddings via \texttt{TextChunkRepository} and \texttt{ChromaService}.
    \item Later, user sends search/query $\rightarrow$ \texttt{\seqsplit{RAGContextService.assemble\_context}} retrieves top chunks using \texttt{ChromaService} or \texttt{SearchService}.
...
The system splits extracted text into sentence-aware chunks within a target size and optionally applies overlap. Each chunk is materialized using the implementation in \texttt{\seqsplit{backend/app/services/text\_extraction\_service.py}}.
...
\For{each sentence $s$ in sentences}
    ...
    \If{current\_word\_count + sentence\_words $>$ max\_chunk\_size \textbf{and} current\_chunk\_sentences not empty}
        ...
        \State chunk\_model $\leftarrow$ \texttt{\seqsplit{TextExtractionService.\_create\_text\_chunk}}(file\_id,
        \Statex \hspace{4em} project\_id, chunk\_text, chunk\_index, start\_pos, current\_word\_count)
...
\textbf{Notes:}
\begin{itemize}
    \item Uses sentence-aware splitting and target size with optional overlap per \texttt{\seqsplit{TextExtractionService}}.
    \item Returns \texttt{TextChunk} Pydantic models; persistence occurs later (e.g., in \texttt{\seqsplit{store\_text\_chunks\_with\_embeddings}}).
\end{itemize}
...
\State embeddings $\leftarrow$ \texttt{\seqsplit{EmbeddingService.generate\_embeddings}}(chunk\_texts)
...
\For{each $(chunk\_info, embedding)$ in \texttt{zip}(list\_of\_chunks, embeddings)}
    \State chunk\_model $\leftarrow$ \texttt{build TextChunk}(\,
        ...
        \Statex \hspace{4em} content $=$ chunk\_info.content,\; start\_position $= \dots$,\; end\_position $= \dots$,
        \Statex \hspace{4em} chunk\_index $=$ chunk\_info.chunk\_index,\; word\_count $= \dots$,
        \Statex \hspace{4em} embedding\_vector $=$ embedding\,)
    \State saved\_chunk $\leftarrow$ \texttt{\seqsplit{TextChunkRepository.create}}(chunk\_model)
...
\If{\texttt{ChromaService} available}
    ...
    \State \texttt{\seqsplit{ChromaService.add\_embeddings}}(project\_id, chroma\_entries)
\Else
...
\begin{itemize}
    \item \textbf{Embedding model:} \texttt{\seqsplit{sentence-transformers/all-MiniLM-L6-v2}} (see embedding service and verification scripts).
    \item \textbf{Dual storage:} Metadata in MongoDB \texttt{text\_chunk} documents; vectors in ChromaDB via \texttt{\seqsplit{backend/app/services/chroma\_service.py}}.
    \item \textbf{Batch background tasks:} Celery jobs generate missing embeddings in \texttt{\seqsplit{backend/app/tasks/indexing.py}} (\texttt{batch\_generate\_embeddings\_task}).
\end{itemize}
...
\State query\_embedding $\leftarrow$ \texttt{\seqsplit{EmbeddingService.generate\_single\_embedding}}(Q)
\If{\texttt{ChromaService} available}
    \State vector\_hits $\leftarrow$ \texttt{\seqsplit{ChromaService.search\_similar}}(project\_id,
    \Statex \hspace{4em} query\_embedding, \texttt{n\_results} $=$ k)
\Else
    \State vector\_hits $\leftarrow$ \texttt{\seqsplit{TextChunkRepository.vector\_search}}(project\_id,
    \Statex \hspace{4em} query\_embedding, \texttt{limit} $=$ k)
\EndIf
...
\If{include\_entities}
    \State entities $\leftarrow$ \texttt{\seqsplit{EntityRepository.get\_by\_project}}(project\_id)
    \State entity\_matches $\leftarrow$ identify entities mentioned in \textit{top\_chunks}
    \Statex \hspace{2em} (\texttt{EntityExtractionService} or string matching)
\EndIf
\If{include\_scenes}
    \State scenes $\leftarrow$ \texttt{\seqsplit{SceneRepository.find\_by\_project}}(project\_id)
    \State mapped\_scenes $\leftarrow$ map chunks to scenes when positions overlap or proximity matches
\EndIf
\If{include\_relationships}
    \State relationships $\leftarrow$ \texttt{\seqsplit{RelationshipRepository.find\_by\_project}}(project\_id)
\EndIf
\State context\_payload $\leftarrow$ \{\texttt{"chunks"}: top\_chunks, \texttt{"entities"}: entity\_matches,
\Statex \hspace{2em} \texttt{"scenes"}: mapped\_scenes, \texttt{"relationships"}: relationships (filtered)\}
...
\textbf{Notes:}
\begin{itemize}
    \item \texttt{\seqsplit{RAGContextService.assemble\_context}} collects chunk-level results, enriches them with entity and scene metadata, and returns a compact context mapping suitable for prompt building (see \texttt{\seqsplit{backend/app/services/rag\_context\_service.py}}).
...
This step orchestrates context assembly, prompt construction, LLM invocation, and persistence. It corresponds to \texttt{\seqsplit{AIChatService.chat}} and \texttt{\seqsplit{CopilotService.generate\_suggestion}}.
...
\State context $\leftarrow$ \texttt{\seqsplit{RAGContextService.assemble\_context}}(\,\texttt{query} $=$ user\_message,
\Statex \hspace{2em} \texttt{project\_id} $=$ project\_id, \texttt{file\_id} $=$ file\_id, \texttt{max\_chunks} $=$ max\_context\_chunks\,)
\State \textbf{Step B: Format context into LLM prompt}
\State formatted\_context $\leftarrow$ \texttt{\seqsplit{RAGContextService.format\_context\_for\_llm}}(context)
\State system\_prompt $\leftarrow$ \texttt{\seqsplit{AIChatService.SYSTEM\_PROMPT}}
\State conversation\_history $\leftarrow$ \texttt{\seqsplit{ConversationRepository.get\_latest\_messages}}(conversation\_id or new)
\State llm\_messages $\leftarrow$ \texttt{\seqsplit{build\_messages\_list}}(system\_prompt, conversation\_history,
\Statex \hspace{2em} user\_message, formatted\_context)
\Comment{system message, recent assistant/user turns, current user message, appended context}
\State \textbf{Step C: Call LLM}
\State assistant\_content $\leftarrow$ \texttt{GroqService.generate}(\texttt{messages} $=$ llm\_messages,
\Statex \hspace{2em} \texttt{max\_tokens} $= \dots$, \texttt{model} $= \dots$)
\State \textbf{Step D: Postprocess and store}
\State cleaned $\leftarrow$ \texttt{clean\_text}(assistant\_content) \Comment{e.g., \texttt{\seqsplit{CopilotService.\_clean\_suggestion}}}
...
\begin{itemize}
    ...
    \item \texttt{CopilotService.generate\_suggestion} targets inline assistance: it infers writing style from RAG results, calls the LLM, then returns a cleaned suggestion and the context used (see \texttt{\seqsplit{backend/app/services/copilot\_service.py}}).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Embedding generation:} Dominated by model inference cost per chunk. Batch generation reduces overhead (indexing background tasks \texttt{\seqsplit{batch\_generate\_missing\_embeddings}}).
    ...
    \end{itemize}
    \item \textbf{Reindexing \& incremental updates:} \texttt{\seqsplit{TextExtractionService.reprocess\_file}} deletes existing chunks and marks the file ready for reprocessing; batch background tasks handle any missing embeddings.
\end{itemize}
...
\State \texttt{\seqsplit{TextChunkRepository.delete\_by\_file}}(file\_id)
\State \texttt{\seqsplit{FileRepository.mark\_ready\_for\_processing}}(file\_id)
\State full\_text $\leftarrow$ \texttt{\seqsplit{TextExtractionService.extract\_text\_from\_file}}(file\_id)
\State chunks $\leftarrow$ \texttt{\seqsplit{TextExtractionService.chunk\_text}}(project\_id, file\_id,
\Statex \hspace{2em} full\_text, \texttt{max\_chunk\_size}, \texttt{overlap})
\State \texttt{TextChunkRepository.bulk\_insert}(chunks)
\State enqueue \texttt{\seqsplit{batch\_generate\_missing\_embeddings}}(project\_id, file\_id)
\end{algorithmic}\normalsize
\end{algorithm}
\FloatBarrier
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/text\_extraction\_service.py}} :: \texttt{\seqsplit{chunk\_text()}}, \texttt{\seqsplit{\_create\_text\_chunk()}}, \texttt{\seqsplit{\_split\_into\_sentences()}}
        \item \texttt{\seqsplit{backend/app/repositories/text\_chunk\_repository.py}} :: \texttt{\seqsplit{create()}}, \texttt{\seqsplit{get\_by\_file()}}, \texttt{\seqsplit{get\_chunks\_without\_embeddings()}}, \texttt{\seqsplit{update\_embedding()}}
    \end{itemize}
    \item \textbf{Embeddings and vector store}
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/embedding\_service.py}} :: \texttt{\seqsplit{generate\_single\_embedding()}}, \texttt{\seqsplit{generate\_embeddings()}}, \texttt{\seqsplit{store\_text\_chunks\_with\_embeddings()}}, \texttt{\seqsplit{find\_similar\_chunks()}}
        \item \texttt{\seqsplit{backend/app/services/chroma\_service.py}} :: \texttt{\seqsplit{get\_or\_create\_collection()}}, \texttt{\seqsplit{add\_embeddings()}}, \texttt{\seqsplit{search\_similar()}}
        \item \texttt{\seqsplit{backend/app/tasks/indexing.py}} :: \texttt{\seqsplit{index\_document\_task()}}, \texttt{\seqsplit{batch\_generate\_embeddings\_task()}}
    \end{itemize}
    \item \textbf{RAG and search}
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/rag\_context\_service.py}} :: \texttt{\seqsplit{assemble\_context()}}, \texttt{\seqsplit{format\_context\_for\_llm()}}
        \item \texttt{\seqsplit{backend/app/services/search\_service.py}} :: \texttt{\seqsplit{semantic\_search()}}, \texttt{\seqsplit{find\_similar\_content()}}, hybrid logic
    \end{itemize}
    \item \textbf{LLM orchestration and assistants}
    \begin{itemize}
        \item \texttt{\seqsplit{backend/app/services/ai\_chat\_service.py}} :: \texttt{\seqsplit{chat()}}
        \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{generate\_suggestion()}}, \texttt{\seqsplit{\_gather\_story\_context()}}
    \end{itemize}
\end{itemize}
}
...
Total Backend Tests Executed & 231 & Integration + Unit across all modules \\ \hline
Passing Tests & 231 &  \\ \hline
Failed Tests & 0 & All tests passing \\ \hline
Overall Pass Rate & 100\% & Fully stable \\ \hline
\end{tabular}
\caption{Overall Backend Testing Summary}
\end{table}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{\seqsplit{test\_track\_accept\_and\_reject\_suggestion}}: Tests suggestion acceptance/rejection tracking
    \end{itemize}
    \item \textbf{Unit Tests (13 test cases):}
    \begin{itemize}
        \item \texttt{\seqsplit{test\_generate\_suggestion\_success}}: Validates end-to-end suggestion generation flow
        \item \texttt{\seqsplit{test\_gather\_story\_context}}: Tests character, location, and theme extraction from project context
        \item \texttt{\seqsplit{test\_analyze\_writing\_style}}: Validates POV, tense, and formality detection
        \item \texttt{\seqsplit{test\_build\_prompt\_continue}}: Tests prompt construction for continue mode with character details
        \item \texttt{\seqsplit{test\_build\_prompt\_includes\_dialogue\_tone\_and\_character\_profile\_details}}: Verifies character attribute injection into prompts
        \item \texttt{\seqsplit{test\_analyze\_writing\_style\_detects\_informal\_tone\_from\_contractions}}: Tests formality detection heuristics
        \item \texttt{\seqsplit{test\_gather\_story\_context\_keeps\_only\_mentioned\_characters\_for\_dialogue}}: Validates context filtering logic
...
\textbf{Backend Pass Rate} & \textbf{100\%} & 231/231 passing \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Pytest configuration:} Tests discovered under \texttt{backend/tests/}; notable files include \texttt{\seqsplit{test\_auth\_api.py}}, \texttt{\seqsplit{test\_projects\_api.py}}, \texttt{\seqsplit{test\_files\_api.py}}, \texttt{\seqsplit{test\_search\_api.py}}, \texttt{\seqsplit{test\_chat\_api.py}}, \texttt{\seqsplit{test\_integration\_workflows.py}}, and \texttt{\seqsplit{test\_health.py}}.
    \item \textbf{Notes:} Built from testing documentation (\texttt{\seqsplit{WHITEBOX\_TESTING\_COMPLETE.md}}, \texttt{\seqsplit{PHASE1\_TESTING\_SUMMARY.md}}, \texttt{\seqsplit{TESTING\_GUIDE.md}}, \texttt{\seqsplit{TEST\_SUMMARY.md}}) and concrete repo files (\texttt{conftest.py}, \texttt{pytest.ini}, run scripts). Exact counts may vary with updates.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Test Patterns:} Test files matching \texttt{src/**/\_\_tests\_\_/**/*} and \texttt{src/**/*.test.*} patterns
...
\begin{itemize}
    \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{CopilotService.generate\_suggestion}}
    \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{CopilotService.\_gather\_story\_context}}
    \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{CopilotService.\_analyze\_writing\_style}}
    \item \texttt{\seqsplit{backend/app/services/copilot\_service.py}} :: \texttt{\seqsplit{CopilotService.\_build\_prompt}}
    \item \texttt{\seqsplit{backend/app/api/v1/endpoints/copilot.py}} :: \texttt{\seqsplit{generate\_suggestion}}
\end{itemize}
}
...
\If{length(last\_sentences) $>$ 20}
    \State query\_embedding $\leftarrow$ \texttt{\seqsplit{EmbeddingService.generate\_single\_embedding}}(last\_sentences)
    \State similar\_chunks $\leftarrow$ \texttt{\seqsplit{EmbeddingService.semantic\_search}}(project\_id,
    \Statex \hspace{4em} last\_sentences, limit $=$ 3, min\_similarity $=$ 0.3)
...
\State story\_context $\leftarrow$ \{mentioned\_characters[:5], mentioned\_locations[0],
\Statex \hspace{2em} mentioned\_themes[:3], style\_samples, style\_characteristics\}
...
\State raw\_suggestion $\leftarrow$ \texttt{\seqsplit{GroqService.generate\_response}}(prompt, temperature $=$ 0.7, max\_tokens)
...
{\small
\begin{itemize}
    \item \texttt{\seqsplit{backend/app/models/entity.py}} :: \texttt{Entity} (with \texttt{EntityType.CHARACTER})
    \item \texttt{\seqsplit{backend/app/repositories/entity\_repository.py}} :: \texttt{\seqsplit{EntityRepository.get\_by\_project\_and\_type}}
    \item \texttt{\seqsplit{backend/app/api/v1/endpoints/genres.py}} :: \texttt{\seqsplit{list\_project\_entities}} (with entity\_type filter)
    \item \texttt{\seqsplit{backend/app/api/v1/endpoints/files.py}} :: \texttt{\seqsplit{process\_uploaded\_file}} (character extraction and update logic)
    \item \texttt{\seqsplit{backend/app/services/entity\_extraction\_service.py}} :: \texttt{\seqsplit{EntityExtractionService.extract\_entities\_from\_text}}
\end{itemize}
}
...
\State file\_record $\leftarrow$ \texttt{\seqsplit{FileRepository.get\_by\_id}}(file\_id)
\State text\_content $\leftarrow$ file\_record.text\_content
\State chunks $\leftarrow$ \texttt{\seqsplit{TextExtractionService.chunk\_text}}(project\_id, file\_id, text\_content)
\State \textbf{Step B: Extract entities from text}
\State extracted\_entities $\leftarrow$ \texttt{\seqsplit{EntityExtractionService.extract\_entities\_from\_text}}(
\Statex \hspace{2em} text\_content, project\_id, file\_id)
...
\For{each entity in extracted\_entities}
    \If{entity.type == CHARACTER}
        \State existing\_entity $\leftarrow$ \texttt{\seqsplit{EntityRepository.get\_by\_name}}(project\_id, entity.name)
        \If{existing\_entity exists}
            ...
            \}
            \State \texttt{\seqsplit{EntityRepository.update\_by\_id}}(existing\_entity.id, update\_payload)
        \Else
...
\State \texttt{\seqsplit{EmbeddingService.store\_text\_chunks\_with\_embeddings}}(project\_id, file\_id, chunks)
\State \textbf{Step E: Discover relationships between characters}
\State \texttt{\seqsplit{RelationshipDiscoveryService.discover\_relationships\_for\_project}}(project\_id)
...
{\small
\begin{itemize}
    \item \texttt{\seqsplit{backend/app/services/web\_search\_service.py}} :: \texttt{\seqsplit{WebSearchService.search}}
    \item \texttt{\seqsplit{backend/app/services/web\_search\_service.py}} :: \texttt{\seqsplit{WebSearchService.format\_results\_for\_llm}}
    \item \texttt{\seqsplit{backend/app/api/v1/endpoints/research.py}} :: \texttt{\seqsplit{research\_chat}}
    \item \texttt{\seqsplit{backend/app/models/conversation.py}} :: \texttt{\seqsplit{ResearchRequest}}, \texttt{\seqsplit{ResearchResponse}}, \texttt{\seqsplit{SourceItem}}
\end{itemize}
}
...
\State conversation $\leftarrow$ \texttt{\seqsplit{AIChatService.\_create\_conversation}}(user\_id, project\_id)
\State \texttt{\seqsplit{AIChatService.\_store\_message}}(conversation.id, role $=$ USER, content $=$ user\_query)
\State assistant\_msg $\leftarrow$ \texttt{\seqsplit{AIChatService.\_store\_message}}(
...
\State \texttt{\seqsplit{ConversationRepository.increment\_message\_count}}(conversation.id)
